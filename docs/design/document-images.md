...
- Nothing else about the file system is reachable. A path that leaves the root, a file that is not an image, and a directory are all refused with a 404.
...
### The root in review mode

The document's own directory by default. `--root <dir>` widens it.
...
`Cache-Control: no-cache` on every image.

- Review mode re-renders when the file changes, and an image regenerated beside it (a re-exported diagram) should show up on the next render rather than after a cache expires.
- Frozen rounds do not freeze images (above), so a long cache for them would be caching something that is not frozen.

## Not in scope

- Remote images (`https://…`) and `data:` URIs. They are left as written.
- Absolute paths (`/abs/x.png`). Left as written, and so they 404 as before: a document that names an absolute path is naming something on the writer's machine, not beside the document.
