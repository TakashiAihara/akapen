...
| review root | a directory under which reviews are keyed by the file's path relative to it, so two hosts with different `$HOME` agree on the key. Kept in the store, not in the environment | `akapen review-root`, `~/.akapen/review-root`, `relativePath` in `review.json` |
