...
| review root | a directory under which reviews are keyed by the file's path relative to it, so two hosts with different `$HOME` agree on the key | `AKAPEN_REVIEW_ROOT`, `--review-root`, `relativePath` in `review.json` |
